...
func createPortMap(specs []string) (*[]Portmap, error) {
	...
	// store the node names as keys and the corresponding port specifications as values
	nodeToPortSpecMap := make(map[string][]string)

	// for each specification, it extracts the node names and the port specifications
